...
def interaction_function(coverages,energies,epsilon,T,F,include_derivatives=False,include_integral=False, first_second_check=True):
    ...
    ######################################################
    # NEW: calculate terms from first-order corrections
    ######################################################
    term_1 = [0]*N_ads
    term_2 = [0]*N_ads
    ...
    for k in range(N_ads):
        q_k = site_lookup[k]
        for s in range(N_sites):
            #print("NSITES {N_sites} {single_sites}".format(**locals()))
            #exit()
            #term_1[k] += epsilon[k] * f[s][q_k]
            term_1[k] = epsilon[k] * f[s][q_k]
    ...
    #for k in range(N_ads):
        ...
        #for s in range(N_sites):
            #for i in idx_lists[s]:
                #eps_theta[k][s] += epsilon[i] * coverages[i]
            #term_2[k] += eps_theta[k][s] * df[s][q_k]

    #print("energies {energies}".format(**locals()))
    #print("term_1 {term_1}".format(**locals()))
    #print("term_2 {term_2}".format(**locals()))

    #E_diff = [a + b + c for a, b, c in zip(energies, term_1, term_2)]
    #E_diff = term_1
    #print("ENERGIES TERM_1")
    #print(energies)
    #print(term_1)

    #for e, t in zip(energies, term_1):
        #print(e, mp.nstr(t, 3), mp.nstr(theta_tot, 3))
    E_diff = energies
    E_diff = [a + 1.00 * b for a, b in zip(energies, term_1)]
    #E_diff = term_2
    #E_diff = [a for a in zip(energies)]
    #E_diff = [0. for a in zip(energies)]
                ...
                ##term is only included if l and k are on the same site
                    ...
                    #for s in range(N_sites):
                        #E_jacob[l][k] += 2*(     \
                             ...
                             #eps_theta_theta[s][q_k]*((df[s][q_k]**2) + \
                                                       #f[s][q_k]*d2f[s][q_k]))
                E_jacob[l][k] += (     epsilon[k] * d2f[s][q_k])
                if l in idx_lists[q_k]:
                    for s in range(N_sites):
                        E_jacob[l][k] += (     epsilon[k] * df[s][q_k])
